...
```{r}
venn.diagram(
        x = list(rownames(MGS_EC), rownames(p2_EC)),
        category.names = c("MGS" , "PICRUSt2"),
        filename = 'EC_venn_diagramm.png',
        output=TRUE,
        imagetype="png" ,
        print.mode = "percent",
        lwd = 2,
...
```{r}
venn.diagram(
        x = list(rownames(MGS_COG), rownames(p2_COG)),
        category.names = c("MGS" , "PICRUSt2"),
        filename = 'COG_venn_diagramm.png',
        output=TRUE,
        imagetype="png" ,
        print.mode = "percent",
        lwd = 2,
...
rm(groups,time)
```
Extrait des matrices de données de MGS et PICRUSt2
MGS
```{r}
for (groups in c("Control Cropland","Amended Cropland","Control Grassland","Amended Grassland")) {
  for (time in c("0","3","51","125")) {
...
```
Calcul des différences entre les groupes de contrôle et de résidus
MGS
```{r}
#Cropland
...
P2_dif_C_3 = P2_mat_Control_Cropland_3 - P2_mat_Amended_Cropland_3
...
P2_dif_G_125 = P2_mat_Control_Grassland_125 - P2_mat_Amended_Grassland_125
```
...
plot_C_0 = ggplot(diag_cor_C_0, aes(x = diag_cor_C_0, y = EC_class, group = EC_class)) +
  ...
  ylab("EC class")
#At time 3
cor_C_3 = cor(t(dif_C_3),t(P2_dif_C_3), method = "spearman")
corr_C_3 = cor.test(t(dif_C_3),t(P2_dif_C_3), method = "spearman")
diag_cor_C_3= diag(cor_C_3)
diag_cor_C_3 = as.data.frame(diag_cor_C_3)
diag_cor_C_3$EC_number = rownames(diag_cor_C_3)
diag_cor_C_3 = merge(diag_cor_C_3, EC_number_class, by = "EC_number")
plot_C_3 = ggplot(diag_cor_C_3, aes(x = diag_cor_C_3, y = EC_class, group = EC_class)) +
  geom_boxplot(fill="gray", alpha=0.2)+
  ggtitle("T3") +
  xlab("Spearman correlation") +
  ylab("EC class")
...
```{r}
table_Crop = data.frame(Rho = c(corr_C_0$estimate,corr_C_3$estimate, corr_C_51$estimate, corr_C_125$estimate),
                        P_value = c(corr_C_0$p.value,corr_C_3$p.value, corr_C_51$p.value, corr_C_125$p.value),
                        Time = c("T0","T3", "T51", "T125"))
rownames(table_Crop) = table_Crop$Time
table_Crop = table_Crop[,-3]
table_Crop = round(table_Crop, 5)
grid_plot_C = grid.arrange(plot_C_0,plot_C_3, plot_C_51, plot_C_125, nrow =2,
...
```{r}
#At time 0
cor_G_0 = cor(t(dif_G_0),t(P2_dif_G_0), method = "spearman")
corr_G_0 = cor.test(t(dif_G_0),t(P2_dif_G_0), method = "spearman")
...
plot_G_0 = ggplot(diag_cor_G_0, aes(x = diag_cor_G_0, y = EC_class, group = EC_class)) +
  ...
  ylab("EC class")
#At time 3
cor_G_3 = cor(t(dif_G_3),t(P2_dif_G_3), method = "spearman")
corr_G_3 = cor.test(t(dif_G_3),t(P2_dif_G_3), method = "spearman")
...
plot_G_3 = ggplot(diag_cor_G_3, aes(x = diag_cor_G_3, y = EC_class, group = EC_class)) +
  ...
  ylab("EC class")
#At time 51
cor_G_51 = cor(t(dif_G_51),t(P2_dif_G_51), method = "spearman")
corr_G_51 = cor.test(t(dif_G_51),t(P2_dif_G_51), method = "spearman")
...
plot_G_51 = ggplot(diag_cor_G_51, aes(x = diag_cor_G_51, y = EC_class, group = EC_class)) +
  ...
  ylab("EC class")
#At time 125
cor_G_125 = cor(t(dif_G_125),t(P2_dif_G_125), method = "spearman")
corr_G_125 = cor.test(t(dif_G_125),t(P2_dif_G_125), method = "spearman")
diag_cor_G_125 = diag(cor_G_125)
diag_cor_G_125 = as.data.frame(diag_cor_G_125)
diag_cor_G_125$EC_number = rownames(diag_cor_G_125)
diag_cor_G_125 = merge(diag_cor_G_125, EC_number_class, by = "EC_number")
plot_G_125 = ggplot(diag_cor_G_125, aes(x = diag_cor_G_125, y = EC_class, group = EC_class)) +
  geom_boxplot(fill="gray", alpha=0.2)+
  ggtitle("T125") +
  xlab("Spearman correlation") +
  ylab("EC class")
...
Save the plot and add table of Rho value & P value
```{r}
table_Grass = data.frame(Rho = c(corr_G_0$estimate, corr_G_3$estimate, corr_G_51$estimate, corr_G_125$estimate),
                        P_value = c(corr_G_0$p.value, corr_G_3$p.value, corr_G_51$p.value, corr_G_125$p.value),
                        Time = c("T0", "T3", "T51","125"))
rownames(table_Grass) = table_Grass$Time
table_Grass = table_Grass[,-3]
table_Grass = round(table_Grass, 5)
grid_plot_G = grid.arrange(plot_G_0, plot_G_3, plot_G_51,plot_G_125, nrow =2,
