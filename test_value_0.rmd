...
for (groups in c("crop","grass")) {
  for (time in c("0","3","51","125")) {
    ...
    plot_cor[[paste0(groups, "_", time)]] = ggplot(d_cor, aes(x = coeff, y = EC_class, group = EC_class, fill = EC_class)) +
      geom_boxplot(alpha=0.2)+
      geom_jitter(color="black", size=0.4, alpha=0.9)+
      geom_vline(xintercept = 0, linetype="dotted", 
                color = "#000000", linewidth=0.4)+
      ggtitle(paste("Correlation of Spearman on data", groups, time)) +
...
plot_grid_Grass <- ggarrange(plot_cor$grass_0, plot_cor$grass_3, plot_cor$grass_51, plot_cor$grass_125,
                            ncol = 2, nrow = 2)
d_cor_control_0 = as.data.frame(d_cor_control_0)
names(d_cor_control_0) = c("coeff")
d_cor_control_0 = d_cor_control_0 %>% filter(coeff > 0)
d_cor_control_3 = as.data.frame(d_cor_control_3)
names(d_cor_control_3) = c("coeff")
d_cor_control_3 = d_cor_control_3 %>% filter(coeff > 0)
d_cor_control_51 = as.data.frame(d_cor_control_51)
names(d_cor_control_51) = c("coeff")
d_cor_control_51 = d_cor_control_51 %>% filter(coeff > 0)
d_cor_control_125 = as.data.frame(d_cor_control_125)
names(d_cor_control_125) = c("coeff")
d_cor_control_125 = d_cor_control_125 %>% filter(coeff > 0)
b = intersect(intersect(rownames(d_cor_control_0),rownames(d_cor_control_3)),
              intersect(rownames(d_cor_control_51),rownames(d_cor_control_125)))
```
```{r}
...
```{r}
threshold = 2
scatter_data <- data.frame(x = as.vector(MGS_EC_imp[a,]), y = as.vector(p2_EC_imp[a,]))
scatter_data$EC_number = rep(rownames(MGS_EC_imp[a,]), ncol(MGS_EC_imp[a,]))
