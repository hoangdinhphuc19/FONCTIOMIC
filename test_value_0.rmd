...
rm(c)
```
Imputation des valeurs nulles + transformation log1p
```{r}
MGS_EC_imp = MGS_EC
...
for (groups in c("Control_Cropland","Amended_Cropland","Control_Grassland","Amended_Grassland")) {
  for (time in c("0","3","51","125")) {
    assign(paste0("cor_", groups, "_", time), 
            cor(t(get(paste0("mat_", groups, "_", time))), t(get(paste0("P2_mat_", groups, "_", time))), method = "pearson"))
    assign(paste0("d_cor_", groups, "_", time), 
            diag(get(paste0("cor_", groups, "_", time))))
        assign(paste0("corr_", groups, "_", time), 
            cor.test(t(get(paste0("mat_", groups, "_", time))), t(get(paste0("P2_mat_", groups, "_", time))), method = "pearson"))
