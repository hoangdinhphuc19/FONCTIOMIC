...
```{r}
meta = read_delim("C:/Users/dphoang/Documents/FONCTIOMIC/FONCTIOMIC/Data FONCTIOMIC/03_dimimos_terrain.csv",delim = ",",col_names = TRUE)
p2_EC = read_delim("C:/Users/dphoang/Documents/FONCTIOMIC/FONCTIOMIC/Data FONCTIOMIC/EC_metagenome_out/pred_metagenome_unstrat.tsv.gz",delim = "\t",col_names = TRUE)
    p2_EC = as.data.frame(p2_EC)
    rownames(p2_EC) = p2_EC[,1]
    p2_EC = as.matrix(p2_EC[,-1])
    p2_EC = round(p2_EC)
p2_KO = read_delim("C:/Users/dphoang/Documents/FONCTIOMIC/FONCTIOMIC/Data FONCTIOMIC/KO_metagenome_out/pred_metagenome_unstrat.tsv.gz",delim = "\t",col_names = TRUE)
    p2_KO = as.data.frame(p2_KO)
    rownames(p2_KO) = p2_KO[,1]
    p2_KO = as.matrix(p2_KO[,-1])
    p2_KO = round(p2_KO)
p2_PW = read_delim("C:/Users/dphoang/Documents/FONCTIOMIC/FONCTIOMIC/Data FONCTIOMIC/pathways_out/path_abun_unstrat.tsv.gz",delim = "\t",col_names = TRUE)
    p2_PW = as.data.frame(p2_PW)
    rownames(p2_PW) = p2_PW[,1]
    p2_PW = as.matrix(p2_PW[,-1])
    p2_PW = round(p2_PW)
otu_table = read_delim("C:/Users/dphoang/Documents/FONCTIOMIC/FONCTIOMIC/Data FONCTIOMIC/03_dimimos_terrain.seq_abundance.16S.tsv",delim = "\t",col_names = TRUE)
otu_table = as.data.frame(otu_table)
row.names(otu_table) = otu_table$sequence
otu_table = as.matrix(otu_table[,-1])
#tax_table = read_delim("",delim = "\t",col_names = FALSE)
phy_tree = read_tree("C:/Users/dphoang/Documents/FONCTIOMIC/FONCTIOMIC/Data FONCTIOMIC/out.tre")
```
Load ref data: mapfile
```{r}
mapfile_EC = read_delim("C:/Users/dphoang/Documents/Data FONCTIOMIC/Inference_fonctionnelle_FONCTIOMIC/dataset_ref/ec_level4_info.tsv.gz",delim = "\t", col_names  = FALSE)
mapfile_KO = read_delim("C:/Users/dphoang/Documents/Data FONCTIOMIC/Inference_fonctionnelle_FONCTIOMIC/dataset_ref/ko_info.tsv.gz",delim = "\t", col_names = FALSE)
mapfile_PW = read_delim("C:/Users/dphoang/Documents/Data FONCTIOMIC/Inference_fonctionnelle_FONCTIOMIC/dataset_ref/metacyc_pathways_info.txt.gz",delim = "\t", col_names = FALSE)
...
p2_EC_pca = vegan::rda(row.names(p2_EC_wide)~.,p2_EC_wide,scale=TRUE)
summary(p2_EC_pca)
```
PCA plot (EC)
```{r}
